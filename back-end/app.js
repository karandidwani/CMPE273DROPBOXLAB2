...
app.post('/doLogin', function (req, res) {
    ...
    kafka.make_request('login_topic', {type: "login", username: username, password: password}, function (err, results) {
        console.log("Results from kafka.make_Request "+results);
        if (err) {
            console.log("Login Failed, received error");
            res.status(500).send({message: "Error in Log in"});
        }
        else {
            if (results.code == 200) {
    ...
    kafka.make_request('login_topic', {type: "uploader", file: req.body.name}, function (err, results) {
        ...
        if (err) {
            ...
            if (results.code == 200) {
                ...
                res.status(401).json({status: 401, message: "Error401"});
            }
        }
    });
});
